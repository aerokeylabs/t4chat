...
const useModelsStore = defineStore('models', () => {
  const models = useLocalStorage<Model[]>('models_list', [
    { id: 'openai/gpt-4o', name: 'GPT-4o', provider: 'OpenAI', capabilities: ['image-uploads', 'pdf-uploads', 'reasoning'], enabled: true },
    { id: 'openai/gpt-4-turbo', name: 'GPT-4 Turbo', provider: 'OpenAI', capabilities: ['image-uploads', 'pdf-uploads', 'reasoning'], enabled: true },
    { id: 'openai/gpt-3.5-turbo', name: 'GPT-3.5 Turbo', provider: 'OpenAI', capabilities: ['reasoning'], enabled: true },
    { id: 'anthropic/claude-3-opus', name: 'Claude 3 Opus', provider: 'Anthropic', capabilities: ['image-uploads', 'pdf-uploads', 'reasoning'], enabled: false },
    { id: 'anthropic/claude-3-sonnet', name: 'Claude 3 Sonnet', provider: 'Anthropic', capabilities: ['image-uploads', 'reasoning'], enabled: true },
    { id: 'anthropic/claude-3-haiku', name: 'Claude 3 Haiku', provider: 'Anthropic', capabilities: ['image-uploads', 'reasoning'], enabled: false },
    { id: 'google/gemini-pro', name: 'Gemini Pro', provider: 'Google', capabilities: ['image-uploads'], enabled: false },
    { id: 'google/gemini-1.5-pro', name: 'Gemini 1.5 Pro', provider: 'Google', capabilities: ['image-uploads', 'pdf-uploads', 'reasoning'], enabled: false },
    { id: 'meta/llama-3-70b-instruct', name: 'Llama 3 70B', provider: 'Meta', capabilities: ['reasoning'], enabled: false },
  ]);
...
const filteredModels = computed(() => {
  return modelsStore.models.filter(model => {
    ...
    const matchesCapabilities = selectedCapabilities.value.length === 0 ||
      selectedCapabilities.value.some(cap => model.capabilities.includes(cap));
    
    // Filter by providers
    const matchesProviders = selectedProviders.value.length === 0 ||
      selectedProviders.value.includes(model.provider);
  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row gap-6">
      ...
      <div class="w-full md:w-64 space-y-6">
        ...
        <div>
          <h3 class="font-medium mb-3">Capabilities</h3>
          <div class="space-y-2">
            <div 
            ...
            >
              ...
              >
                {{ capability === 'pdf-uploads' ? 'PDF Uploads' :
                   capability === 'image-uploads' ? 'Image Uploads' :
        ...
        <div>
          <h3 class="font-medium mb-3">Providers</h3>
          <div class="space-y-2">
            <div 
                ...
                <div>
                  ...
                  <div class="flex gap-2 mt-2">
                    <span 
                      ...
                      :key="capability"
                      class="bg-secondary px-2 py-0.5 rounded-full text-xs"
                    >
                      {{ capability }}
                    </span>
                  </div>
